...
export function VisitorRequestModal({ leaderId, onClose }: Props) {
  ...
  // Reset state every time the modal closes (leaderId === null) so the
  ...
  // Render the Turnstile widget the first time the modal opens. On subsequent
  // opens we reset() the existing widget instead of recreating it. The widget
  // is removed only when the component itself unmounts (page navigation),
  // not on every modal close — that avoids "widget not found" warnings from
  // Turnstile's own DOM-removal watcher.
  useEffect(() => {
    if (!leaderId) {
      if (widgetIdRef.current && window.turnstile) {
        try {
          ...
          /* widget may have errored out — fine */
        }
      }
      return;
    }
  ...
  // Final teardown: only when the component itself unmounts.
  useEffect(() => {
    return () => {
      if (widgetIdRef.current && window.turnstile) {
        try {
  ...
  return (
    ...
    >
      <div className="bg-white dark:bg-zinc-900 rounded-xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto p-5 md:p-6 text-zinc-950 dark:text-zinc-50">
        ...
        {/* Form state — kept mounted always so Turnstile widget stays in DOM */}
